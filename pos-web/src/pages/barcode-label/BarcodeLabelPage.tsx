...
import type { BarcodeLabelSize, Product } from "../product/ProductTypes";
...
export default function BarcodeLabelPage() {
  ...
  const selectedProducts = useMemo(
    () => products.filter((product) => selectedIds.includes(product.id) && product.barcode),
    [products, selectedIds],
  );
  const printableLabels = selectedProducts.flatMap((product) =>
    ...
    <MainLayout>
      <div className="space-y-6">
        <Card className="flex flex-col justify-between gap-4 p-5 sm:flex-row sm:items-center">
          ...
          <Button onClick={() => void fetchProducts()}><RefreshCw className="h-4 w-4" /> Refresh</Button>
        </Card>
          ...
          <Card className="p-5">
            <div className="flex items-center justify-between gap-3">
              ...
              <Button
                ...
                onClick={() => setSelectedIds(products.filter((product) => product.barcode).map((product) => product.id))}
              >
                Pilih Semua
              </Button>
            </div>
